...
type Tab = 'mean-reversion' | 'momentum'

const TABS: { id: Tab; icon: string; label: string }[] = [
  { id: 'mean-reversion', icon: '↩', label: 'Mean Reversion' },
  { id: 'momentum',       icon: '↑', label: 'Momentum VCP' },
]

export default function EntrySetups() {
  const [searchParams, setSearchParams] = useSearchParams()
  const tab = (searchParams.get('tab') as Tab) ?? 'mean-reversion'
  ...
  return (
    <div>
      ...
      <Suspense fallback={<Loading />}>
        {tab === 'mean-reversion' && <MeanReversion />}
